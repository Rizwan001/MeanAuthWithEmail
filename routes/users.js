...
router.post('/forgetPassword', (req, res) => {
  ...
  User.getUserByEmail(email, (err, user) => {
   ...
   if(!user){
    ...
    const output = `
    <p>Registration Detail</p>
      Hi ${user.username},<br/> Thank you for registering! <br/><br/> Please verify your email by typing the following token On the following page:<a href="http://localhost:3000/reset/${resetPasswordToken}">Password Reset</a>  <br/><br/>
    `;
...
/* ============================================================
    Route to check if user's id is available for registration
 ============================================================ */
 router.get('/getUserById/:id', (req, res) => {
    ...
    User.getUserById({ _id: req.params.id }, (err, user) => {
      if (err) {
        ...
        // Check if user's e-mail is taken
        if (user) {
          res.json({ success: true, message: user}); // Return as taken e-mail
        } else {
          res.json({ success: false, message: 'No user found' }); // Return as available e-mail
        }
      }
    });
...
/* ============================================================
    Route to check if user's token is available for registration
 ============================================================ */
